...
test_page = page_metadata[0]["path"]

img = Image.open(test_page).convert("RGB")

print("\n[EMBED TEST]")
print("Original:", img.size)

processor = embedder._first_module().processor
texts = [""]
processed = processor(text=texts, images=img, return_tensors="pt")

print("Processed:", processed["pixel_values"].shape)

print("\n[EMBEDDER PROCESSOR INFO]")
...
try:
    ...
    print("Processor inspect failed:", e)


def encode_query(query: str):

    emb = embedder.encode(query, prompt="Represent the user's input.", normalize_embeddings=True)
...
def search_pages(query: str, emb: np.ndarray, top_k: int = 30) -> list:
    ...
    scores, indices = page_index.search(emb, 30)

    results = []

    print(f"\nTop-30 pages from embedding search:\n")
...
def rerank_candidates(query: str, candidates: list, model, processor, device: str) -> list:
    ...
    for i, cand in enumerate(candidates):
        print(
            f"{i+1:02d}. "
            f"Folder={cand['folder']} | "
            f"Page={cand['page']} | "
            f"RetrievalScore={cand.get('score', 0):.4f}"
        )
    reranked = []
        ...
        for cand in batch:
            if cand["type"] in ("page", "image"):
                try:
                    ...
                    try:
                        processed = rerank_processor.image_processor(img, return_tensors="pt")

                        print("Tensor shape:", processed["pixel_values"].shape)

                    except Exception as e:
                        print("Processor debug failed:", e)
    ...
    print(f"\n[RERANK OUTPUT]")
    for i, cand in enumerate(reranked[:20]):
        print(
            f"{i+1:02d}. "
            f"Folder={cand['folder']} | "
            f"Page={cand['page']} | "
            f"RerankScore={cand['rerank_score']:.4f} | "
            f"RetrievalScore={cand.get('score', 0):.4f}"
        )
...
def full_pipeline_only_pages_only_embedder(query: str, timeout_seconds: int = 180):
    ...
    expanded_pages = sorted(
        expanded_pages, key=lambda x: x.get("rerank_score", x.get("score", 0)), reverse=True
    )[:6]
