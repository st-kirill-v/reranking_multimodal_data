...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a filtered clean Qwen3 tile index by skipping low-information visual crops."
    )
    parser.add_argument("--data-dir", type=Path, default=Path("data/datasets/docbench"))
    parser.add_argument("--index-dir", type=Path, default=Path("index"))
    parser.add_argument("--index-name", default="tiles_qwen3_docapi_grid2x3_filtered")
    ...
    parser.add_argument("--min-ink-ratio", type=float, default=0.01)
    parser.add_argument("--min-contrast", type=float, default=5.0)
    parser.add_argument("--min-entropy", type=float, default=0.45)
    parser.add_argument("--min-edge-density", type=float, default=0.002)
...
def skip_reasons(stats: dict, args: argparse.Namespace) -> list[str]:
    reasons = []
    if stats["ink_ratio"] < args.min_ink_ratio:
        reasons.append("low_ink")
    if stats["contrast"] < args.min_contrast:
        reasons.append("low_contrast")
    if stats["entropy"] < args.min_entropy:
        reasons.append("low_entropy")
    if stats["edge_density"] < args.min_edge_density:
...
def main() -> None:
    ...
    tile_position_counter: Counter[str] = Counter()
    start = time.time()
    ...
    for page_record in tqdm(page_records, desc="Embedding filtered tiled pages"):
        with Image.open(page_record["path"]) as img:
            size = img.size
        for tile_record in tile_records_for_page(
            page_record,
            image_size=size,
            rows=args.rows,
            cols=args.cols,
            ...
            total_tiles += 1
            tile_position_counter[str(tile_record["tile_id"])] += 1
            tile = load_tile(tile_record)
            stats = image_stats(tile)
            reasons = skip_reasons(stats, args)
            if reasons:
                skipped_tiles += 1
                ...
                continue
            kept_tiles += 1
            batch_records.append({**tile_record, "quality": stats})
    ...
    if index is None:
        raise RuntimeError("No embeddings were produced after filtering")
    ...
    manifest = {
        ...
        "tiling": {
            ...
            "candidate_tiles": total_tiles,
            "kept_tiles": kept_tiles,
            "skipped_tiles": skipped_tiles,
            "kept_rate": kept_tiles / max(total_tiles, 1),
            "skip_reasons": skip_counter.most_common(),
            "tile_position_counts": tile_position_counter.most_common(),
            "filter": {
    ...
    if validation["stored_vs_fresh_mean"] is None or validation["stored_vs_fresh_mean"] < 0.95:
        raise SystemExit("Filtered tile index validation failed: stored_vs_fresh_mean is too low.")
